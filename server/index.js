...
// GET /api/ebay/import/scan — fetch active, sold, ended listings from eBay for import preview
app.get('/api/ebay/import/scan', async (req, res) => {
  try {
    const token = await getValidAccessToken(req.companyId);
    const xml = `<?xml version="1.0" encoding="utf-8"?>
<GetMyeBaySellingRequest xmlns="urn:ebay:apis:eBLBaseComponents">
  ...
  <HideVariations>true</HideVariations>
    ...
    const soldSection = body.match(/<SoldList>([\s\S]*?)<\/SoldList>/)?.[1] || '';
    for (const m of soldSection.matchAll(/<Transaction>([\s\S]*?)<\/Transaction>/g)) {
      const b = m[1];
      const itemBlock = b.match(/<Item>([\s\S]*?)<\/Item>/)?.[1] || '';
      sold.push({
        itemId: getField(itemBlock, 'ItemID'),
        title: getField(itemBlock, 'Title'),
        price: getField(b, 'TransactionPrice') || getField(b, 'SalePrice') || getField(itemBlock, 'CurrentPrice'),
        conditionId: getField(itemBlock, 'ConditionID') || '3000',
        soldDate: getField(b, 'CreatedDate') || getField(b, 'LastModifiedTime'),
        images: getPictures(itemBlock),
        status: 'sold',
      });
    }

    // Parse ended/unsold listings
    const endedSection = body.match(/<UnsoldList>([\s\S]*?)<\/UnsoldList>/)?.[1] || '';
    for (const m of endedSection.matchAll(/<Item>([\s\S]*?)<\/Item>/g)) {
      const b = m[1];
      ended.push({
        itemId: getField(b, 'ItemID'),
        title: getField(b, 'Title'),
    ...
    console.log(`[import/scan] Results — active: ${active.length}, sold: ${sold.length}, ended: ${ended.length}`);

    // Mark items already imported into this company
    const db = await getDb();
    const allItemIds = [...active, ...sold, ...ended].map(i => i.itemId).filter(Boolean);
    const existingDocs = allItemIds.length > 0
      ? await db.collection('listings').find(
          { companyId: req.companyId, ebayDraftId: { $in: allItemIds } },
          { projection: { ebayDraftId: 1 } }
        ).toArray()
      : [];
    const existingIds = new Set(existingDocs.map(d => d.ebayDraftId));
    const mark = items => items.filter(i => i.itemId).map(i => ({ ...i, alreadyImported: existingIds.has(i.itemId) }));

    res.json({ active: mark(active), sold: mark(sold), ended: mark(ended) });
  } catch (e) {
    console.error('[import/scan] error:', e.message);
    res.status(500).json({ error: e.message });
  }
});

// POST /api/ebay/import/execute — fetch full details for selected items and save to DB
app.post('/api/ebay/import/execute', async (req, res) => {
  ...
  try {
    ...
    const CONDITION_MAP = { '1000': 'New', '1500': 'New Other', '2000': 'Manufacturer Refurbished', '2500': 'Like New / Open Box', '3000': 'Used', '4000': 'Very Good', '5000': 'Good', '6000': 'Acceptable', '7000': 'For Parts or Not Working' };
    ...
    for (const item of items) {
      try {
        // Fetch full item details via GetItem
        const xml = `<?xml version="1.0" encoding="utf-8"?>
<GetItemRequest xmlns="urn:ebay:apis:eBLBaseComponents">
  ...
  <DetailLevel>ReturnAll</DetailLevel>
        ...
        const b = resp.data;
        const get = tag => { const r = new RegExp(`<${tag}[^>]*>([\\s\\S]*?)<\\/${tag}>`); return r.exec(b)?.[1]?.trim() || ''; };

        // Description — strip CDATA wrapper if present
        const rawDesc = get('Description');
        const description = rawDesc.replace(/^<!\[CDATA\[/, '').replace(/\]\]>$/, '').trim();

        // Item specifics
        const itemSpecifics = {};
        for (const m of b.matchAll(/<NameValueList>([\s\S]*?)<\/NameValueList>/g)) {
          const name = m[1].match(/<Name>([\s\S]*?)<\/Name>/)?.[1]?.trim();
          const value = m[1].match(/<Value>([\s\S]*?)<\/Value>/)?.[1]?.trim();
          if (name && value) itemSpecifics[name] = value;
        }

        // Images — use GetItem's PictureURL if available, otherwise fall back to scan data
        const imageUrls = [...b.matchAll(/<PictureURL>(.*?)<\/PictureURL>/g)]
          .map(m => m[1]).filter(u => u.startsWith('http'));
        const images = imageUrls.length > 0 ? imageUrls : (item.images || []);

        const conditionId = get('ConditionID') || item.conditionId || '3000';
        const condition = CONDITION_MAP[conditionId] || 'Used';
        const title = get('Title') || item.title || '';
        const category = get('CategoryName') || get('CategoryID') || item.category || '';
        const priceRaw = parseFloat(get('CurrentPrice') || get('BuyItNowPrice') || item.price || '0');
        const createdAt = item.startTime ? new Date(item.startTime).getTime() : Date.now();
        const now = Date.now();

        const listing = {
          id: crypto.randomUUID(),
          ebayDraftId: item.itemId,
          title,
          description,
          condition,
          category,
          priceRecommendation: priceRaw > 0 ? `$${priceRaw.toFixed(2)}` : '',
          shippingEstimate: '',
          itemSpecifics,
          images,
          tags: [],
        ...
        if (effectiveStatus === 'sold') {
          ...
          listing.status = 'listed'; // active — not archived
        }

        await createListing(req.companyId, listing);
        imported++;

        // Small delay to avoid eBay rate limits on large batches
        await new Promise(r => setTimeout(r, 80));
      } catch (err) {
        console.error(`[import/execute] Failed itemId=${item.itemId}:`, err.message);
        failed.push({ itemId: item.itemId, title: item.title, reason: err.message });
      }
    }

    res.json({ imported, skipped: failed.length, failed });
  } catch (e) {
    console.error('[import/execute] error:', e.message);
    res.status(500).json({ error: e.message });
  }
});
