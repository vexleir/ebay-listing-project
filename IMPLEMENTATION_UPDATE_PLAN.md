...
**Current implementation phase:** MOB-001 (a–e) ✓, FE-004 follow-through ✓, INV-002 lite ✓, UX-002 focus-trap ✓, UX-002 lightbox a11y ✓, IMG-001 lite ✓, IMG-002 lite ✓, FE-003 test backfill ✓, IMG-001 crop ✓, IMG-001 aspect-ratio presets ✓, Quota middleware test backfill ✓, IMG-001 straighten ✓, IMG-003 slice 1 ✓, IMG-003 slice 2 ✓, ImageEditModal test backfill ✓, IMG-001 auto-enhance ✓, applicationToken test backfill ✓, INV-001 slice 1 ✓, INV-002 inventory-badge wire-up ✓, INV-002 inventory bootstrap on stage ✓, INV-002 push counter sync ✓, INV-002 sold-sync counter wiring ✓, AI quota admin controls ✓, INV-002 end-listing counter wiring ✓, ImageEditModal upload bugfix ✓, **INV-002 warn-before-push gate ✓** (PushToEbayModal blocks duplicate listings with an acknowledgment checkbox when the SKU is already live; INV-002 user-facing loop now complete), **INTEL-001 slice 1 ✓** (Phase 4 schema + push/relist snapshot capture + read-only `/api/intelligence` REST), **INTEL-002 slice 1 ✓** (outcome capture schema + composite-id-idempotent upsert + `POST /api/intelligence/outcomes/capture` orchestrator).

### Second-engagement consultant review remediation (2026-05-31) — P0 + P1 complete

The follow-up review ([CONSULTANT_REVIEW_2026-05-30.md](CONSULTANT_REVIEW_2026-05-30.md)) flagged two data-correctness risks, a lint regression, and four finish items. All P0 and P1 items are now done:

- **P0.1 — deprecated Finding API sold comps.** `server/optimizer.js` `fetchSoldComps` (Finding API `findCompletedItems`, deprecated by eBay) replaced with `fetchActiveComps` over the Browse API (the supported source the Repricing Advisor already used). `/api/optimizer/comps` now returns `mode: 'active'`. All "sold comps" UI relabeled to "active market comps" (`SoldCompsPanel`, `StagedListings` toasts, `StagedListingActions` label + tests, shared `CompsPanel` already honest). `fetchSoldComps` kept as a backward-compat alias.
- **P0.2 — sold-history lookback never wired to the client.** `App.handleSyncSold` now passes `?lookbackDays=` (persisted 30/60/90 choice); auto-sync uses 90 to avoid silently dropping sales when the app is closed >30 days. New lookback selector + manual "Sync Sold" button added to the Sold tab (empty state + toolbar). `src/components/SoldListings.test.tsx` (5 cases) locks the wiring.
- **P0.3 / P1.1 — lint gate restored.** `eslint.config.js` ignores `archive/`, downgrades `@typescript-eslint/no-explicit-any` and the experimental React-compiler rules (`purity`/`refs`/`set-state-in-effect`) to warnings, keeping real errors hard. Fixed the genuine bugs: `useListFilterSort` set-state-in-effect → render-time clamp; `csv.ts` irregular-whitespace BOM literal → `\uFEFF`; StagedListings conditional `useEffect` (rules-of-hooks) moved above the early return; `no-empty` / `no-unused-expressions` cleanups. Lint now exits 0 (**0 errors, 80 warnings**). Added `lint:strict` (`--max-warnings 0`) and a `verify` script (`lint && build && test:all`).
- **P1.2 — bundle code-split.** The eleven heavy tabs (`BulkUploader`, `StagedListings`, `ListedProducts`, `SoldListings`, `Analytics`, `SettingsPanel`, `ListingOptimizer`, `AdminPanel`, `EbayImportTab`, `HelpPage`, `Feedback`, `ContainerManagement`) are `React.lazy` + `Suspense`. Main bundle **659.94 KB → 262 KB** (gzip 164 KB → 79 KB); the >500 KB warning is gone.
- **P1.3 — ContainerManagement extraction.** Merge/Split/Move-items modals extracted to `containers/ContainerActionModals.tsx` (stateless, parent-owned state).
- **P1.4 — server-side sold reconciliation.** New `services/ebay/soldItems.js` (shared fetch/parse, used by both the route and the scheduler), `services/ebay/soldReconciler.js` (per-company + all-company orchestration with isolated failures + sold-milestone outcome capture), `listings.reconcileSoldListings` (DB mark-sold), and `services/ebay/soldReconcilerScheduler.js` (30-min interval, opt out via `SOLD_RECONCILE_ENABLED=false`) wired into `index.js`. Sales now reconcile even with no browser tab open.
- **P1.6 — AI model-selection dedup.** Shared `services/ai/modelSelect.js` replaces the duplicated model-discovery/preference logic in `ai.js` and `optimizer.js`.

New tests: `model-select.test.js` (9), `sold-reconciler.test.js` (10), `SoldListings.test.tsx` (5). Frontend tests: **369 passing across 40 files**. Server tests: **521 passing**. Build clean; lint 0 errors.
...
| INTEL-002 (slice 1) | Done | Claude | 2026-05-29 | **Outcome capture schema + orchestrator + REST endpoint. Idempotent at the (experimentId, milestone) composite id, so a daily polling task can re-run "capture 7d for everything published 6–8 days ago" without dedup logic.** Added `server/services/intelligence/outcome.js` — pure `buildOutcomeSnapshot()` builder plus helpers `normalizeMilestone` (case-insensitive trim, returns `null` for unknown), `toIntOrNull` (numbers + numeric strings, floors, rejects negatives), `normalizePrice` (strips `$`/whitespace), `daysSince` (clamps negatives to 0 for clock-skew safety, returns `null` on invalid input), and the frozen `VALID_MILESTONES = ['publish', '7d', '14d', '30d', 'sold', 'end']`. Schema: `id` = `<experimentId>:<milestone>` (composite — re-firing the same milestone overwrites instead of duplicating), `companyId`, `experimentId`, `listingId`, `ebayItemId`, `captureMilestone`, `capturedAt`, `ageDays` (integer days since `experiment.publishedAt`, `null` when publishedAt is missing/invalid), `viewCount`, `watcherCount`, `quantitySold`, `soldAt`, `finalSalePrice`, `activePrice`, `status`. Throws on missing experiment / `experiment.id` / `experiment.companyId` / `milestone`. Non-finite stats are silently nulled rather than thrown so a partial GetItem response doesn't tear down the daily capture loop. **17 unit tests** in `intelligence-outcome.test.js` lock in the frozen set + the helpers + every required field, the sold-milestone shape with `finalSalePrice` + `soldAt`, the composite-id idempotency invariant, the missing-publishedAt → null `ageDays` rule, and the throws on bad input. Extended `server/intelligence.js` with `upsertOutcome` (idempotent at composite id, defensive companyId-mismatch guard), `getOutcome`, `listOutcomesForExperiment` (sorted asc by `capturedAt` — the Optimizer impact panel will walk this), `listOutcomesForCompany({ milestone, limit, since })`. Added a new `listing_outcomes` collection constant. **15 new CRUD tests** in `intelligence.test.js` cover the upsert, the overwrite-at-same-id invariant, the companyId-stamp + mismatch guard, the cross-tenant isolation paths, the milestone + since filters, and the limit. The fake-Mongo `collection()` helper in the test file grew an `updateOne` with `upsert: true` semantics. Added `server/services/intelligence/captureOutcome.js` — high-level orchestrator `captureOutcomeForEbayItem(companyId, ebayItemId, { milestone, stats, status }, deps)` that injects `getExperimentByEbayItemId` + `upsertOutcome` + `now`. Returns `{ skipped: true, reason: 'no-experiment' }` when no experiment matches (i.e., the listing was pushed before INTEL-001 went live) so the caller can keep aggregate counters honest without a thrown error tearing down the surrounding sync loop. **6 unit tests** in `intelligence-capture-outcome.test.js` cover the happy path, the now() injection (verified the `ageDays` floor math), the skip path, the missing-arg throws, the missing-deps throws, and the milestone-error surfaced from the builder. Extended `server/routes/intelligence.js`: `GET /outcomes` (`?milestone=&limit=&since=`, newest first), `GET /outcomes/by-experiment/:experimentId` (every milestone for one experiment, sorted asc by `capturedAt` — for the future Optimizer impact panel), `GET /outcomes/:id` (composite-id fetch), `POST /outcomes/capture` (body `{ ebayItemId, milestone, stats, status }`, returns 201 on a fresh write, 204 when no experiment exists for the item — so a cron/sync loop can ignore the response, 400 on missing fields). **10 new route tests** in `routes-intelligence.test.js` cover the list + filters + 404s + 201 + 204-no-experiment + 400-missing-fields + the idempotent-re-fire-overwrites invariant (write twice at `7d`, list returns one row with the second value). **NOT shipped this slice (deliberately deferred — these are integration with already-running flows, not new orchestration)**: (1) the scheduled-capture cron that walks experiments and fires `7d` / `14d` / `30d` on the appropriate dates — no scheduler infrastructure exists in the server yet, so adding it touches startup/shutdown and is its own ticket; (2) the sold-sync hook that auto-fires `milestone='sold'` from the existing `GET /api/ebay/sold-items` pagination (the most valuable single milestone — knowing the final sale price closes the loop on every experiment — but requires careful idempotency in the existing route which is the longest non-test file in the codebase). Both have a stable API to land on now: the captureOutcome service + the REST `POST /outcomes/capture` endpoint. Server tests **435 passing** (was 387 + 48 new — 17 builder + 15 CRUD + 6 orchestrator + 10 route); frontend tests unchanged at 356; build clean. |
| INTEL-001 (slice 1) | Done | Claude | 2026-05-29 | **Phase 4 kickoff — listing experiment schema + push/relist snapshot capture.** Added `server/services/intelligence/snapshot.js` — pure `buildExperimentSnapshot()` builder plus helpers `normalizePrice` (strips `$`/whitespace, returns null for non-finite / non-numeric), `uniqueStrings` (deduped trimmed strings), and `toIntCount` (count of numbers / array length / object keys). Schema: `id`, `companyId`, `listingId`, `ebayItemId`, `source` (`'push' \| 'relist'`), `createdAt`, `publishedAt`, `promptVersion`, `optimizerVersion`, `listingScoreAtPublish`, `titleLength`, `categoryId` (prefers `pushContext.categoryId` over `listing.categoryId`), `categoryName`, `priceAtPublish`, `shippingPolicyId`, `bestOfferEnabled`, `itemSpecificsCount`, `imageCount`, `tags`. Throws on missing `id`/`companyId`/`listing`/`ebayItemId`. **16 unit tests** in `intelligence-snapshot.test.js` lock in the helpers + the canonical doc shape + the source defaulting (anything not `'relist'` becomes `'push'`) + the missing-optional-fields tolerance + the `pushContext.categoryId` priority + the throws + the NaN-score-becomes-null path. Added `server/intelligence.js` — tenant-scoped Mongo CRUD with `createExperiment` (defensive companyId-mismatch check), `getExperiment`, `getLatestExperimentForListing` (sorted by `publishedAt` desc — sellers can re-push the same listing, the latest is what INTEL-002 outcome attribution targets), `getExperimentByEbayItemId` (used by the INTEL-002 outcome webhook so eBay's item ID can find the matching experiment without knowing the local listingId), and `listExperimentsForCompany({ limit, since })`. **18 CRUD tests** in `intelligence.test.js` cover the persistence path, the cross-tenant isolation rules, the latest-for-listing ordering, the by-item-id stringification, and the limit + `since` filters. Hooked into `POST /api/ebay/draft` AND `POST /api/ebay/relist` in `server/routes/ebay/lifecycle.js` — after `pushListingToEbay` returns success, both routes build the snapshot and call `createExperiment` inside a try/catch so a Mongo blip or a missing prompt version only loses the experiment row, not the successful push. The `/draft` hook tags `source: 'push'` and pulls `pushContext` from the request body (`overrideCategoryId`, `overrideFulfillmentPolicyId`, `bestOffer`); the `/relist` hook tags `source: 'relist'` and pulls from the live-GetItem merge (`liveCategoryId`, `livePolicies.shipping`). `promptVersion` is read from `listing.tokenUsage.promptVersion` (set by `services/ai/prompts.js` via AI-001), `optimizerVersion` from `listing.optimizerVersion`, `listingScoreAtPublish` from `listing.healthScore`. **7 integration tests** in `ebay-push-experiment.test.js` cover the /draft happy path (asserts every field on the persisted doc), the /draft no-write-on-push-throw, the /draft non-fatal-on-experiment-throw, the /draft missing-listing tolerance, the /draft tenant pass-through, the /relist source=relist write, and the /relist non-fatal-on-throw. Added `server/routes/intelligence.js` — read-only REST mounted at `/api/intelligence`. Endpoints: `GET /experiments` (`?limit=N&since=ISO`, sorted by `publishedAt` desc), `GET /experiments/by-listing/:listingId` (the latest), `GET /experiments/by-ebay-item/:ebayItemId`, `GET /experiments/:id`. Writes intentionally absent — the snapshot is derived from the listing state at the moment of publish and shouldn't be hand-crafted by clients. **12 route tests** in `routes-intelligence.test.js` cover auth gating, the list / by-listing / by-ebay-item / by-id paths including 404 handling and tenant isolation. **Unblocks INTEL-002 (outcome capture), INTEL-003 (optimizer action tracking), and INTEL-004 (Optimizer impact panel in Analytics)** — those slices now have a stable schema to write outcomes against and to read snapshots from. Server tests **387 passing** (was 334 + 53 new — 16 snapshot + 18 CRUD + 7 push-hook + 12 routes); frontend tests unchanged at 356; build clean. |
| INV-002 warn-before-push gate | Done | Claude | 2026-05-29 | **Block accidental duplicate listings at the push confirmation step.** Added two helpers to `src/utils/duplicateSku.ts`: `isLiveListing(listing)` (SKU set + `status === 'listed'` + not sold) and `findLiveListingConflicts(sku, allListings, currentListingId?)` (returns OTHER live items sharing the SKU). Staged drafts are intentionally NOT counted — pushing a staged item is exactly what flips it to listed, so flagging staged copies would create noise. The lite warn-on-edit path (`findConflictingListings`) keeps treating staged collisions as conflicts because they matter at type-time. **10 new unit tests** cover the live-only predicate (staged false, sold-listed false, blank-SKU false) and the conflict finder (excludes staged duplicates, excludes sold rows, excludes own id, case-insensitive, blank-SKU returns empty). Extended `PushToEbayModal` (`src/components/staged/PushToEbayModal.tsx`) with a new `liveSkuConflicts?: StagedListing[]` prop. When non-empty, the modal renders a red `role="alert"` panel with the matching listing titles (first 3 + "+N more"), eBay item IDs when present, and an acknowledgment checkbox ("Push anyway — I understand this will create a duplicate listing"). The Push button is `disabled` (with an explanatory `title`) until the checkbox is ticked. The acknowledgment state is reset whenever the conflict set OR the listing identity changes via `useEffect`, so the gate can't silently stay green across multiple staged items. **7 new component tests** in `PushToEbayModal.test.tsx` cover the no-conflict no-render, the single-conflict panel content (title + eBay ID), the plural-conflict copy, the +N more truncation at 3 entries, the disabled-until-ack rule, the re-enable-after-tick path, and the no-conflict-doesn't-disable safeguard. Wired into `src/components/StagedListings.tsx` — the modal callsite now computes `findLiveListingConflicts(pushModal.listing.sku, allListings ?? listings, pushModal.listing.id)` and passes it as the new prop. Uses `allListings` (staged + listed pool) so live items in the Listed tab show up, not just the staged-only `listings` array. **INV-002 user-facing surface now closes the loop**: warn-on-type (DuplicateSkuWarning), warn-on-stage (InventoryItemBadge), warn-before-push (this slice). Frontend tests **356 passing across 38 files** (was 339 + 17 new — 10 helper + 7 modal); server tests unchanged at 334; build clean. |
| ImageEditModal upload bugfix | Done | Claude | 2026-05-29 | **Fix "Failed to save images: {}" when saving from the EditListingModal image editor.** Root cause: `src/components/ImageEditModal.tsx` was stripping the `data:image/...;base64,` prefix off the FileReader output (`.split(',')[1]`) before POSTing to `/api/images/upload`. Cloudinary's `uploader.upload()` requires the full data URI when given a base64 payload; receiving just the raw base64 caused the SDK to throw an error whose `.message` was undefined. The route's `res.status(500).json({ error: e.message })` then serialized to `{}` (JSON.stringify drops undefined values), the frontend wrapped that in `new Error(await resp.text())`, and the toast displayed "Failed to save images: {}". Fix: stop stripping the prefix — send the raw `reader.result as string`. Also tightened both error paths so a future Cloudinary failure won't be invisible: (1) the route now derives a usable `detail` string from `e.message ?? e.error.message ?? e.http_code ?? 'Image upload failed (no detail returned)'` before sending; (2) the modal now throws `new Error('Image upload failed (${status} ${statusText}): ${body}')` so the toast surfaces the HTTP status; (3) added a defensive `if (!Array.isArray(data?.urls))` shape check so an unexpected response shape produces a clear error rather than a downstream undefined. The pre-existing IMG-003 slice 1 + slice 2 commits both had this bug (they extracted the broken implementation from the original inline ImageEditModal in StagedListings, which had the same prefix-stripping). Sellers who only added NEW images without editing existing ones may have been catching some related symptoms but never reported because the toast wording masked the root cause. Verified with `npm run build` (clean), frontend tests **339 unchanged**, server tests **334 unchanged**. |
| INV-002 end-listing counter wiring | Done | Claude | 2026-05-29 | **Auto-restore inventory when a listing is moved back to staged (the explicit "Move to Staged" / end-without-sale flow).** Added a pure `detectListedToStagedTransition(existing, updates)` to `server/services/inventory/soldTransition.js`. Returns `{ sku, deltas: { quantityOnHand: 1, quantityListed: -1 } }` on the listed→staged boundary, `null` otherwise. **Three intentional bail-outs**: (a) when the same PUT also marks the item sold, defer to the sold transition (which already decrements listed, so firing both would double-count); (b) when the existing record is already in the sold bucket (`soldAt` is truthy), moving the archived row back to staged should NOT restore an on-hand unit; (c) when the existing record has no SKU or only whitespace. **8 new unit tests** in `sold-transition.test.js` cover the happy path, no-status-in-updates, not-currently-listed, not-going-to-staged, both-sold-and-status defer-to-sold rule, already-sold no-op, no-SKU no-op, and null/non-object inputs. Wired into `PUT /api/listings/:id` in `server/routes/listings.js` alongside the existing sold-transition logic — the route now loads the existing record once when either `'soldAt' in updates` OR `'status' in updates` and runs both detectors. The move-to-staged counter fires inside a try/catch so a Mongo hiccup only loses the bookkeeping tick, not the listing update. **4 new integration tests** in `listings-sold-sync.test.js` cover the happy path (status=staged on a listed item fires `{ quantityOnHand: 1, quantityListed: -1 }`), the staged→staged no-op, the both-sold-and-status defer-to-sold rule (only the sold delta lands), and the resilience-on-counter-throw. **The INV-002 counter pipeline is now complete for all three core transitions**: push success (listed↑/on-hand↓), mark-sold (sold↑/listed↓ + the unmark inverse), and end-listing/move-to-staged (on-hand↑/listed↓). Server tests **334 passing** (was 322 + 12 new — 8 detector + 4 integration); frontend tests unchanged at 339; build clean. |
| AI quota admin controls | Done | Claude | 2026-05-29 | **Sellers can now adjust or disable the per-day AI token quota from the Settings panel.** Background: the backend already read `settings.aiDailyTokenLimit` per company (falling back to `AI_DAILY_TOKEN_LIMIT` env, then the hard-coded 100,000 default) — but there was no UI to set it, so admins had no in-app way to override the cap. Backend changes: extended `getAiDailyQuotaStatus(companyId)` in `server/listings.js` to include a `disabled` flag (mirrors `settings.aiQuotaDisabled`); `remainingTokens` returns `Number.POSITIVE_INFINITY` when disabled so any downstream consumer that compares against it naturally allows the call. Modified `enforceAiDailyQuota` in `server/middleware/quota.js` to short-circuit to `return true` when `quota.disabled` is set — the 429 path is skipped entirely. **2 new quota-middleware tests** in `server/tests/quota.test.js` cover the disabled-bypass (returns true even with 0 remaining and a 999,999-token reserve, no body written) and the explicit-`disabled:false` still-enforces case (covers a future regression where the flag default flips silently). Frontend changes: added `aiDailyTokenLimit?: number` and `aiQuotaDisabled?: boolean` to `UserSettings` in `src/types/index.ts`. Added a new **"AI Token Quota"** section to `src/components/SettingsPanel.tsx` directly below the existing AI Settings panel: a number input for the daily limit (blank = use system default, `min=0`, `step=1000`, disabled while the toggle is on, `aria-label`), a checkbox to disable the cap entirely (`aria-label`), and a warning banner that appears when disabled so the seller doesn't forget the cap is off. Per-call rate limits (which prevent any single user from spamming the AI endpoint) still apply — the change only removes the daily-cumulative 429. The existing `/api/settings` POST already accepts arbitrary fields, so no settings-route changes were needed. **The 100,000 default is now an override-able floor**: keep it for safety, change it to fit a Gemini API tier, or turn it off if the seller pays Google directly. Server tests **322 passing** (was 320 + 2 new); frontend tests unchanged at 339; build clean. |
| INV-002 sold-sync counter wiring | Done | Claude | 2026-05-29 | **Auto-update inventory counters when a listing flips sold (and when it's un-marked).** Added `getListing(companyId, id)` to `server/listings.js` (tenant-scoped single-doc fetch — missing piece needed to detect the "was-not-sold → is-now-sold" transition). Added the pure `server/services/inventory/soldTransition.js` module with `isSoldState(doc)` (truthy `soldAt` — positive number, non-blank string) and `detectSoldTransition(existing, updates)`. The detector returns `null` for any update that doesn't cross the sold boundary so the route doesn't pay for a Mongo round-trip on price-only / image-only edits. **12 unit tests** in `sold-transition.test.js` cover the truthiness rules for `soldAt` (positive number / non-blank string / null / undefined / 0 / empty string / whitespace), the four combinations of (was-sold, will-be-sold), the no-SKU bail-out, the "already-sold → still-sold-but-different-timestamp" no-op, and the explicit-undefined unmark path. Wired into `PUT /api/listings/:id` in `server/routes/listings.js` — when `updates` contains `soldAt`, the route loads the existing record via `getListing`, calls `detectSoldTransition`, applies the update, and (when crossing) fires `incrementInventoryCounters` with the deltas inside a try/catch so the counter sync stays **non-fatal**. The mark-sold path emits `{ quantitySold: +1, quantityListed: −1 }`; the unmark-sold path emits `{ quantitySold: −1, quantityListed: +1 }`. **7 new integration tests** in `listings-sold-sync.test.js` cover the mark-sold happy path, the unmark-sold happy path, the no-counters-when-soldAt-absent rule, the no-counters-when-soldAt-changes-but-state-same rule (updating a sale date keeps the row sold so no transition fires), the no-counters-when-no-SKU rule, the resilience-on-counter-throw (200 still returned), and the multi-tenant pass-through. **INV-002 counter pipeline is now complete for the two highest-volume transitions** (push → listed↑, mark-sold → sold↑/listed↓). Future slice: the end-listing path (when a seller explicitly ends a listing without selling, restore one to on-hand). Server tests **320 passing** (was 301 + 19 new — 12 detector + 7 integration); frontend tests unchanged at 339; build clean. |
| INV-002 push counter sync | Done | Claude | 2026-05-29 | **Auto-increment inventory counters on push-to-eBay success.** Added `incrementInventoryCounters(companyId, sku, deltas)` to `server/inventory.js` — atomic `$inc` over `quantityOnHand` / `quantityListed` / `quantitySold`. Ignores non-numeric / non-finite deltas, truncates fractional values to integers, returns the post-increment doc (or `null` when no matching SKU). Re-reads after the `$inc` so the caller sees the post-update counters regardless of Mongo driver version. **9 new unit tests** cover the atomic adjust path, the updatedAt refresh, the null-on-miss case, the silent-skip-of-bad-deltas rule, the fractional truncation, the empty-deltas no-op, the case-insensitive SKU lookup, the missing-companyId/SKU throws, and the cross-tenant isolation (a `co2` call against `co1`'s X is a no-op and doesn't touch the `co1` row). Updated the fake Mongo collection in `inventory.test.js` to support `$inc` and return `matchedCount` alongside `modifiedCount`. Hooked into `POST /api/ebay/draft` in `server/routes/ebay/lifecycle.js` — after `pushListingToEbay` returns success, if the listing carries a non-blank SKU, the route fires `incrementInventoryCounters` with `{ quantityOnHand: -1, quantityListed: 1 }` inside a try/catch so the counter update is **non-fatal**: a missing inventory row or a Mongo hiccup only loses the bookkeeping tick. **6 new integration tests** in `ebay-push-inventory.test.js` cover the happy-path increment, the absent-SKU no-op, the blank/whitespace-SKU no-op, the no-counter-touch-when-push-throws path, the resilience-on-counter-throw (200 still returned), and the multi-tenant pass-through. Uses Module.prototype.require patching for the listingLifecycle service + inventory + rate-limit middleware to avoid touching real eBay / Mongo. **Push counter sync now meaningful** — the listed count goes up when a seller pushes; the on-hand count goes down. The badge component shipped earlier reflects this on every reload. **Future slices**: the sold-sync path (when a listing flips to sold → +1 sold, −1 listed) and the end-listing path (−1 listed, +1 on-hand) are not yet wired. Server tests **301 passing** (was 286 + 15 new — 9 helper + 6 integration); frontend tests unchanged at 339; build clean. |
| INV-002 inventory bootstrap on stage | Done | Claude | 2026-05-29 | **Auto-create an inventory record when a listing is staged with a SKU.** Added `ensureInventoryItemForSku(companyId, sku, defaults?)` to `server/inventory.js` — idempotent: returns the existing item when one already holds the SKU (case-insensitive across tenants); only inserts when no record exists yet. Caller-supplied defaults (quantityOnHand defaults to 1, costBasis/sourceTag/sourceEvent honored on new items) seed the new row. Throws on missing companyId. Returns `{ item, created }` so callers can distinguish "first stage" from "restock". **6 new unit tests** in `inventory.test.js` cover the new-item creation, the no-op-when-exists path that preserves the existing quantityOnHand, the case-insensitive collision, the defaults honoring, the cross-tenant isolation (co1's "X" doesn't block co2 from creating its own), and the missing-companyId throw. Hooked into `POST /api/listings` in `server/routes/listings.js` — after `createListing` succeeds, if the staged listing carries a non-blank SKU, the route fires `ensureInventoryItemForSku` (forwarding `listing.costBasis` as a default) inside a try/catch so the inventory bootstrap is **non-fatal**: the listing is already saved, and a bootstrap failure only logs a warn line and suppresses the badge UX. **6 new integration tests** in `listings-inventory-bootstrap.test.js` cover the happy path (POST with SKU triggers the bootstrap), the costBasis forwarding, the absent-SKU no-op, the blank/whitespace-SKU no-op, the resilience-on-bootstrap-throw path (200 still returned), and the multi-tenant pass-through. Each test uses `Module.prototype.require` patching for both the listings and inventory modules so we exercise the wiring without booting Mongo. **The badge component shipped earlier now lights up automatically** — sellers don't need to manually create inventory records; the first stage of any new SKU seeds the record, and the badge appears on every subsequent stage of the same SKU. Server tests **286 passing** (was 274 + 12 new — 6 ensure helper + 6 bootstrap integration); frontend tests unchanged at 339; build clean. |
| INV-002 inventory-badge wire-up | Done | Claude | 2026-05-29 | **Frontend consumes the new `/api/inventory/by-sku/:sku` endpoint.** Added `src/hooks/useInventorySkuLookup.ts` — debounced (default 300ms) lookup against the inventory endpoint. Returns `{ item, loading, error }`. Treats a 404 as "not found" (not an error), URL-encodes the SKU, cancels the previous request when the SKU changes mid-debounce, and clears state immediately when the SKU goes blank. Exports the `InventoryItem` type so the badge component can re-use it. **11 hook tests** in `useInventorySkuLookup.test.ts` cover the idle paths (empty/whitespace SKU), the loading-immediate flag, the happy-path fetch + Bearer auth header + URL encoding, the 404-as-not-found rule, the 500-becomes-error rule, the network-rejection-becomes-error rule, the cancel-on-rerender path (with fake timers), the no-fetch-on-unmount path, and the clear-on-blank-SKU reset. Added `src/components/InventoryItemBadge.tsx` — informational badge (NOT a warning — duplicate-SKU conflicts already fire `DuplicateSkuWarning`) that displays `displayedSku`, on-hand / listed / sold counts, and the source tag. Renders nothing when item=null + not loading; shows a "Checking inventory…" placeholder when loading=true unless `hideLoading` is set (the typing path uses this so the badge doesn't flicker per keystroke). Carries an `aria-label` summarizing the counts for screen readers. **7 component tests** in `InventoryItemBadge.test.tsx` cover the null no-render, loading no-render with `hideLoading`, loading-shows-with-default, full render with all counts, displayedSku fallback to normalized sku, sourceTag conditional, and the aria-label. Wired into `src/components/ResultsEditor.tsx` (the new-listing flow) and `src/components/EditListingModal.tsx` (listed-item edit), both directly under the existing `DuplicateSkuWarning`. **This is the first frontend consumer of the INV-001 endpoint** — the durable inventory truth is now visible to sellers as they type a SKU. Future INV-002 work (warn-before-push gating, merge-duplicate-records, quantity-aware push) builds on the same hook. Frontend tests **339 passing across 38 files** (was 321 + 11 hook + 7 badge); server tests unchanged at 274; build clean. |
| INV-001 (slice 1) | Done | Claude | 2026-05-29 | **Backend inventory schema + CRUD + REST endpoints.** Added `server/services/inventory/validate.js` — pure helpers (`normalizeInventorySku`, `coerceNonNegativeInt`, `coerceCurrencyString`, `validateInventoryItem`, `buildInventoryItemDoc`, `buildInventoryItemUpdates`) so the schema rules are testable without Mongo. Schema is one document per physical SKU per company: `id` (caller-supplied UUID), `companyId` (tenant key), `sku` (normalized lowercase), `displayedSku` (original casing), `quantityOnHand`, `quantityListed`, `quantitySold`, `costBasis` (currency-as-string), `sourceTag`, `sourceEvent`, `createdAt`, `updatedAt`. Added `server/inventory.js` — Mongo CRUD with `createInventoryItem` (rejects duplicate SKU within the same company → 409), `getInventoryItem`, `getInventoryItemBySku` (case-insensitive lookup, tenant-scoped — useful for the warn-before-staging UX), `listInventoryItems({ skuPrefix?, sourceTag? })`, `updateInventoryItem` (rejects SKU rename that collides → 409), `deleteInventoryItem`. Errors carry `e.status` (400 / 409) so route handlers can map them directly to HTTP. **44 unit + integration tests**: `inventory-validate.test.js` (23 cases — every helper, default-fill, currency-string stripping, validation success/failure paths, SKU renormalization on updates) and `inventory.test.js` (21 cases — full CRUD + the cross-tenant isolation paths, case-insensitive collision detection, no-op rename, no-op update returns existing doc, prefix filter). The CRUD tests use `Module.prototype.require` patching to inject a fake `./db` with an in-memory `Map<companyId::id, doc>` store. Added `server/routes/inventory.js` and mounted it at `/api/inventory` below the global authMiddleware in `app.js`. Endpoints: `GET /` (list, with `?skuPrefix=` / `?sourceTag=` filters), `GET /by-sku/:sku` (404 when missing), `GET /:id`, `POST /` (201; auto-generates an id via `crypto.randomUUID()` when omitted), `PUT /:id` (404 when missing), `DELETE /:id`. Errors with `e.status` propagate verbatim; everything else becomes 500. **16 integration tests** in `routes-inventory.test.js` cover auth gating (401), the empty-list response, create happy-path + auto-id + missing-body + blank-SKU + duplicate-SKU (409), get-by-id 404, get-by-sku, the tenant-isolation path (a token for `co1` cannot see `co2`'s inventory), update happy-path + missing-updates + 404 + 409-on-collision, delete happy-path + 404. **Frontend consumption (the eventual full INV-002: warn-before-push, merge-duplicate-records, quantity-aware push gating) is NOT shipped this slice** — that's a separate backend → frontend wiring effort. The lite SKU warning shipped under INV-002 lite still reads from the in-memory listings array; it can be migrated to read this new endpoint in a follow-up. Server tests **274 passing** (was 214 + 60 new); frontend tests unchanged at 321; build clean. |
| applicationToken test backfill | Done | Claude | 2026-05-28 | **10 server-side unit tests for `server/services/ebay/applicationToken.js`** — the in-memory cache for the eBay Browse API client-credentials OAuth token. Previously untested beyond integration coverage through the sold-comps route. Added `server/tests/application-token.test.js` covering: throws on missing `EBAY_CLIENT_ID` / `EBAY_CLIENT_SECRET`; fetches a fresh token on the first call; POSTs `grant_type=client_credentials` with the URL-encoded `https://api.ebay.com/oauth/api_scope` scope value (asserts the encoded `%3A%2F%2F` so a refactor that drops `URLSearchParams` would catch); sends the `Basic <base64(client_id:client_secret)>` Authorization header with `Content-Type: application/x-www-form-urlencoded`; reuses the cached token across multiple calls inside the validity window (one POST regardless of N calls); refreshes after expiry (using a short `expires_in` so the 60s safety margin pushes it into already-expired territory); honors the 60s safety margin (with `expires_in=120`, the second call inside the same tick still serves from cache); `__resetApplicationTokenCache` forces a refetch; transport errors propagate. Uses an injectable `transport` (already documented as the test seam in the production module) — no Module.prototype.require patching needed. The test snapshots and restores `process.env.EBAY_CLIENT_ID` / `EBAY_CLIENT_SECRET` around each case so leakage doesn't pollute later tests in the same `node --test` invocation. Server tests **214 passing** (was 204 + 10 new); frontend tests unchanged at 321; build clean. |
| IMG-001 auto-enhance | Done | Claude | 2026-05-28 | **Brightness / contrast / saturation sliders — final IMG-001 piece.** Added `src/utils/imageEnhance.ts` with the pure helpers `clampPercent(value)` (±100 window, 0 on non-finite), `isIdentity(opts)`, `cssFilterFor(opts)` (returns `"none"` at identity, otherwise `"brightness(X) contrast(Y) saturate(Z)"` for the live preview), and `applyEnhancements(rgbaBuffer, opts)` (in-place: brightness = additive shift around 255, contrast = (px - 128) × factor + 128, saturation = interpolate around Rec.601 luma). The async `enhanceImageFile(file, opts)` short-circuits at identity (returns the original File), otherwise canvas-decodes → `getImageData` → `applyEnhancements` → `putImageData` → PNG export. **18 unit tests** in `imageEnhance.test.ts` cover the clamp window + non-finite handling, the identity shortcut, the CSS-filter string output, and the per-pixel math (brightness saturates at full intensity, negative brightness drives to 0, +50 contrast expands around 128, −100 contrast collapses to 128, −100 saturation grayscales via Rec.601 luma, alpha preserved, no-op on empty buffer, multi-pixel batch). Added `src/components/ImageEnhanceModal.tsx` (~115 lines) — portal-rendered, focus-trapped, Escape-dismissable modal with three sliders (-100/+100 each, step 1, value display, `aria-label` announces current value), live CSS-filter preview that matches the canvas math (no canvas churn while dragging — only on Save), per-axis Reset button. Wired a 5th button (`Sliders` icon, `aria-label="Enhance image"`) into the `ImageEditModal` per-thumbnail row. Shrunk all five buttons from 20px to 18px and the gap from 3px to 2px so the row still fits in the 110px-wide thumbnail (5×18 + 4×2 = 98px). Updated the existing `ImageEditModal.test.tsx` aria-label enumeration to include the new button. **IMG-001 is now complete** apart from the unified Image Studio modal refactor (consolidating the five edit affordances into one surface with tabs) — that's a larger redesign and not scoped here. Frontend tests **321 passing across 36 files** (was 303 + 18 new); build clean. |
| ImageEditModal test backfill | Done | Claude | 2026-05-28 | **9 component tests for the post-IMG-003-slice-2 image modal**, locking in the unified-items model + dialog behavior. Added `src/components/ImageEditModal.test.tsx` covering: header + existing-image rendering, the empty-state drop-zone copy, the MAIN-label on the first slot, single-thumbnail removal (re-validates that the new first slot inherits MAIN), the per-thumbnail edit-button aria-labels (later extended to 5 buttons when IMG-001 auto-enhance shipped), the Close-button → onClose wiring, the URL-only Save fast-path (no `/api/images/upload` fetch, onSave receives the existing list in order), the "X existing · Y pending upload" counter line, and a drag-and-drop reorder test that yields to the macrotask queue to clear the `setTimeout(0)` from the production `onDragStart` handler before reading `draggedIdx`. The test spies on `URL.createObjectURL` / `URL.revokeObjectURL` so future regressions in the blob-URL ownership tracking get caught. **Did NOT cover** the canvas-dependent edit-button paths (Crop / Rotate / Straighten / Enhance / Scissors actually applying their edit) — those would require a happy-dom canvas mock for `toBlob` + `getImageData`. They remain manually verified. |
| IMG-003 slice 2 | Done | Claude | 2026-05-28 | **Per-thumbnail Crop / Rotate / Straighten / Scissors inside `ImageEditModal`.** Added `src/utils/urlToFile.ts` with `urlToFile(url, fallbackName)` (fetches a URL into a File so the existing canvas-based edit pipeline can act on URL-backed images) and the pure helper `deriveFileName(url)` (handles query strings, fragments, data: URIs, empty input). **6 tests** in `urlToFile.test.ts` cover the normal path, query-string + fragment stripping, the data:-URI / empty / trailing-slash empty fallbacks. Refactored `ImageEditModal` from a two-list (existing URLs + new Files) model to a single ordered `items: ItemSlot[]` list where each slot is either url-backed or file-backed. This makes per-thumbnail edits preserve position — when a seller edits an existing EPS-hosted image, the edited File replaces the URL slot in place instead of being appended at the end. The thumbnail grew from 90×90 to 110×110 to fit the new edit row, and each item now exposes four buttons (`Crop`, `Straighten`, `RotateCw`, `Scissors`) along the bottom edge with `aria-label`s on each. `Crop` opens the existing `ImageCropModal` against a freshly-loaded File; `Straighten` opens `ImageStraightenModal`; `RotateCw` runs `rotateImageFile` synchronously (no sub-modal); `Scissors` POSTs the base64 to the existing `/api/images/remove-bg` and wraps the returned data: URL back into a File. A per-item `busyId` state disables the row while an edit is in flight. Blob: URLs created by file-backed items are tracked via `blobUrlOwned` and revoked on unmount + on each replace. The Save flow rebuilds the output array in item order — pass-through URLs stay, file-backed slots get uploaded to Cloudinary via `/api/images/upload` and their URLs are slotted in. **External contract preserved**: `onSave(images: string[])` unchanged, so the `StagedListings` and `EditListingModal` callsites still work without modification. Frontend tests **294 passing across 34 files** (was 288 + 6 new); build clean. |
| IMG-003 slice 1 | Done | Claude | 2026-05-28 | **Edit/add/remove/reorder images on listed items + push the new photo set to eBay.** Pulled the existing inline `ImageEditModal` out of `src/components/StagedListings.tsx` into a shared `src/components/ImageEditModal.tsx` so the listed-item flow can reuse the same drag-reorder + drop-zone surface. Backend: added `server/services/ebay/reviseImages.js` (`isAlreadyOnEps`, `resolveReviseImageUrls`, `EPS_HOST_FRAGMENTS`) — splits the caller's image list into pass-through EPS URLs (`i.ebayimg.com`, `thumbs.ebaystatic.com`) and the rest that must round-trip through `uploadImagesToEps`, then re-merges in the original order so slot 0 stays the main image. `POST /api/ebay/revise` now accepts `images` and `listingId`; when `images` is present and non-empty, the resolved EPS URLs are emitted as `<PictureDetails>` inside the `ReviseFixedPriceItemRequest` XML body, and after a successful revise the local DB record is best-effort updated via `updateListing(req.companyId, listingId, { images: resolvedImageUrls })` so the next modal-open doesn't need a `refresh-images` round-trip. The response includes `imageUrls` so the frontend can mirror the resolved EPS URLs into its local state. Skips the picture block entirely when `images` is omitted, so unrelated revise calls (price/title/condition edits) leave the live photo set untouched. **8 server tests** in `server/tests/revise-images.test.js` cover the EPS host detection (positive / negative / non-string safety), the EPS-only no-upload path, the mixed-source path (only non-EPS uploaded, output preserves original order with uploaded URLs slotted in correctly), and the upload-error-propagates path. Frontend: extended `src/components/EditListingModal.tsx` with an Images row above Title — a 64×64 thumbnail strip (first 8 + "+N more" overflow), an "Edit images…" button that opens the shared `ImageEditModal`, and an `imagesDirty` flag that drives a "Modified" badge and gates whether `images` + `listingId` are sent on `Save & Push to eBay`. `buildUpdates()` also includes the new images list so Save-to-app-only persists the change locally. The save success path mirrors the server's `imageUrls` back into the local listing record. Added a one-line warning under the strip explaining that eBay treats `PictureDetails` as a full replacement so the seller doesn't accidentally wipe the live set. **Slice 1 deliberately reuses `ImageEditModal` as-is** — per-thumbnail Crop / Rotate / Straighten / Scissors buttons inside the sub-modal are deferred to a follow-up slice. Frontend tests **288 passing across 33 files** (unchanged — UI tests for the new modal section are a follow-up); server tests **204 passing** (was 196 + 8 new); build clean. |
...
### Handoff notes for the next developer (2026-05-29)

The session that just ended landed INTEL-001 + INTEL-002 slice 1 and then was interrupted before INTEL-003. The state below is what's already in `main`; nothing is mid-edit on disk.

**What's done and merge-ready:**

- **INTEL-001 slice 1.** The `listing_experiments` collection has a stable schema, a pure builder (`server/services/intelligence/snapshot.js`), CRUD (`server/intelligence.js`), and read-only REST (`/api/intelligence/experiments`). The push success hook in `server/routes/ebay/lifecycle.js` writes one experiment row per `POST /api/ebay/draft` and per `POST /api/ebay/relist`, both non-fatally. `source` is `'push'` vs `'relist'`. **The hook reads `promptVersion` from `listing.tokenUsage.promptVersion`** — AI-001 sets this; if you add a new AI call path, mirror that field or the experiment row will show `promptVersion: null`. **`optimizerVersion` and `healthScore` are read from `listing.optimizerVersion` and `listing.healthScore`** — neither field exists on the listing model yet (see "Open seams" below). They land as `null` today, by design — the schema is forward-compatible.

- **INTEL-002 slice 1.** The `listing_outcomes` collection has a builder (`server/services/intelligence/outcome.js`), idempotent upsert (`upsertOutcome` in `server/intelligence.js` — composite id `<experimentId>:<milestone>`, so re-firing the same milestone overwrites instead of duplicating), an orchestrator (`server/services/intelligence/captureOutcome.js`), and routes (`GET /api/intelligence/outcomes`, `/outcomes/by-experiment/:id`, `/outcomes/:id`, `POST /outcomes/capture`). The orchestrator returns `{ skipped: true, reason: 'no-experiment' }` when no experiment matches the eBay item id (legacy listings pushed before INTEL-001 went live) so a sync loop doesn't tear down on the first miss. **`POST /outcomes/capture` returns 204 in that case** — a cron can ignore the response without treating absence as an error.

**Open seams to be aware of:**

1. **The scheduled-capture task is NOT shipped.** INTEL-002 slice 1 lays the API but doesn't run the cron that fires `7d` / `14d` / `30d` at the right calendar offsets. The server has no scheduler infrastructure today (no `node-cron`, no `bull`, no `agenda`); adding one touches startup/shutdown in `server/bootstrap.js` and is its own ticket. A pragmatic first cut: a periodic `setInterval` inside `bootstrap()` that walks `listExperimentsForCompany` per-tenant and calls `captureOutcomeForEbayItem` for any experiment whose age has crossed an uncaptured milestone. Cleaner: write a separate worker process. Either is acceptable — pick based on infra. **Until that exists, outcomes can still be captured by an external poller hitting `POST /outcomes/capture`.**

2. **The `'sold'` milestone is NOT auto-fired yet.** The existing `GET /api/ebay/sold-items` in `server/routes/ebay/sync.js` is the obvious wiring point — when the sold-pagination loop finds an item, also fire `captureOutcomeForEbayItem({ milestone: 'sold', stats: { finalSalePrice, soldAt, quantitySold }, status: 'completed' })`. This is the most valuable single milestone (final sale price closes the loop on every experiment). The reason it was held back this slice: `sync.js` is one of the longest files in the codebase and the sold-pagination already coordinates `saveSettings`, dedup, and tenant scoping — wiring the hook carefully (idempotent, non-fatal) deserves its own focused pass.

3. **`listing.healthScore` and `listing.optimizerVersion` aren't persisted yet.** The INTEL-001 hook reads them defensively so the schema is ready, but `server/listings.js` doesn't write either field. The optimizer route in `server/routes/optimizer.js` produces a score in-memory but doesn't tag the listing with it. Next slice that touches the optimizer should mirror `tokenUsage.promptVersion` into `optimizerVersion` and persist the score into `healthScore` so the experiment row is populated end-to-end.

4. **INTEL-003 is unblocked and was next on the queue.** It needs an `optimizer_actions` collection (schema documented in Phase 4 of the plan), wired into the optimizer's "Apply suggestion" path so each accepted AI recommendation produces an action row. The CRUD pattern is identical to `server/intelligence.js` — same `getDb` import, same Module.prototype.require test pattern, same tenant-scoping rules. Put it in `server/intelligence.js` (the file already owns experiments + outcomes; actions belong in the same domain) or split a new module if you prefer one collection per file.

5. **INTEL-004 (Optimizer impact panel in Analytics) needs INTEL-003 to be meaningful.** It can render outcome data on its own (publish vs 7d delta charts) but the "did the optimizer help?" question requires action rows to join against.

**Things to NOT change without thinking carefully:**

- **The composite id format for outcomes is `<experimentId>:<milestone>`.** Idempotency relies on this; if you change the separator or the milestone string the polling task can dup rows.
- **The non-fatal try/catch around `createExperiment` and `incrementInventoryCounters` in the push routes.** Removing those would let a Mongo blip fail a successful eBay push, which would break the seller's mental model ("eBay shows the listing but the app says it failed?").
- **The `companyId`-mismatch guards in `createExperiment` / `upsertOutcome`.** A forged body could otherwise write into another tenant's space. Tests cover this — keep them.
- **`server/intelligence.js` exports both `COLLECTION` and `OUTCOMES_COLLECTION`.** The future migration / index work in Section 7 of the plan should reference these constants rather than hard-coding the strings.

**Test counts at handoff:** Server **435 passing**, frontend **356 passing across 38 files**, `npm run build` clean. The summary line at the top of this tracker stays accurate as you ship; please update it as part of every ticket commit so other developers can see the current truth without scrolling.
