...
**Current implementation phase:** MOB-001 essentially complete (a/b/c/d/e); FE-004 follow-through landed (SoldListings adopted `useListFilterSort`). Only the PWA shell remains under MOB-001 and the plan says to defer it until the broader responsive layout is stable — which it now is. Frontend tests: **175 passing across 22 files**. Next high-leverage tickets: IMG-001 (in-browser crop/rotate), INV-001 (inventory item model), or the eBay-API discovery sessions for OFFER-001/FULFILL-001.
...
**Phase 0 ✓, Phase 1 backend ✓, Phase 2 ✓, Phase 1 frontend ✓, MOB-001 (a/b/c/d/e) ✓ — full mobile pass landed apart from the optional PWA shell.** Server tests: 188 passing. Frontend tests: **175 passing across 22 files** (Vitest + happy-dom + testing-library). All three large screens remain below the 400-line plan budget: `StagedListings.tsx` **760**, `ListedProducts.tsx` **543**, `ListingOptimizer.tsx` **381**. On phones: sidebar is a hamburger-triggered drawer; the New-Listing grid no longer breaks; Uploader/ResultsEditor are touch-friendly; the SoldListings/Optimizer 3-up tiles collapse to 2 then 1 column; Push-to-eBay/Optimizer push diff/Listed-tab Optimize modals keep their action row stuck to the bottom; the BulkUploader's panel paddings collapse on phones.
...
1. **IMG-001 (in-browser crop/rotate/straighten)** — Phase 3 Image Studio v1. The mobile shell is now stable so this can land without fighting layout regressions. Highest immediate user value.
2. **INV-001 (inventory item model)** — Phase 3 inventory/SKU truth. Adds the underlying schema; UI follows in INV-002. Backend-heavy, can run in parallel with IMG-001.
3. **PWA shell** — optional polish on top of the now-stable responsive layout. Plan says this is fine to start at any point now that MOB-001a–e are done.
4. **UI-001 follow-through** — As each subcomponent is touched (mobile pass or otherwise), replace inline style objects with the shared classes (`.list-row`, `.modal-backdrop`, `.modal-card`, `.tabs-strip`, `.filter-bar`, `.empty-state`, `.metric-cell`, `.badge`, `.btn-danger`). Don't bundle this with unrelated structural extraction.
