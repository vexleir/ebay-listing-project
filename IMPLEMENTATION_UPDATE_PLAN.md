...
**Current implementation phase:** MOB-001 (a–e) ✓, FE-004 follow-through ✓, INV-002 lite ✓, UX-002 focus-trap ✓, UX-002 lightbox a11y ✓, IMG-001 lite ✓, IMG-002 lite ✓, **FE-003 test backfill ✓** (the three untested optimizer subcomponents — Header / ScoreGrid / EditForm — now have 26 cases between them). Only the PWA shell remains under MOB-001. Frontend tests: **255 passing across 31 files**. Remaining IMG-001 scope: crop, straighten, auto-enhance. Other high-leverage tickets: full INV-001/INV-002 scope, OFFER-001/FULFILL-001 discovery, INTEL-001 (experiment schema).
...
1. **IMG-001 full (crop + straighten + auto-enhance + unified Image Studio modal)** — Phase 3 Image Studio v1. The rotate slice + duplicate-detection shipped; crop is the next biggest user-visible win.
2. **INV-001 + full INV-002** — Phase 3 inventory/SKU truth. Adds the underlying inventory-item schema (quantity on-hand vs listed vs sold, source-tag, cost basis) plus the warn-on-push, merge-duplicate-records, and quantity-aware-rules upgrades. The INV-002 *lite* slice (warn-on-edit using the existing `sku` field) is in; the full scope needs the schema first.
